            ...
            <CardContent>
                ...
                <div className={"grid grid-cols-1 lg:grid-cols-3 gap-4"}>
                    <InputContainer label={"Objective"}>
                        <Input value={state.objective?.toUpperCase()} disabled/>
                    </InputContainer>
                    <InputContainer label={"Budget Type"}>
                        <Input value={state.budget?.type.toUpperCase()} disabled/>
                    </InputContainer>
                    <InputContainer label={"Budget Value"}>
                        <Input value={`R$ ${formater(state.budget?.amount ?? 0)}`} disabled/>
                    </InputContainer>
                </div>

                <div className={"grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4 mb-4"}>
                    <InputContainer label={"Start date"}>
                        <Input value={state.budget?.schedule_start} disabled/>
                    </InputContainer>
                    <InputContainer label={"End date"}>
                        <Input value={state.budget?.schedule_end} disabled/>
                    </InputContainer>
                </div>
